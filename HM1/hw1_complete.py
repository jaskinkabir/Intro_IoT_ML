...
age_np = np.array(list_of_ages)
height_np = np.array(list_of_heights_cm)

average_age = np.mean(age_np)
average_height = np.mean(height_np)

plt.scatter(age_np, height_np)
...
def classify_iris(x):
    global weights
    global biases
    return np.argmax(weights @ x + biases)
...
classifier.train(x_data, y_labels, validation_data=(x_data, y_labels), epochs=100)
...
def random_training():
    ...
    while accuracy < 0.65:
        ...
        weights = np.random.rand(3,4)
        biases = np.random.rand(3)
        accuracy, _ = evaluate_classifier(classify_iris, x_data.to_numpy(), y_labels.to_numpy(), False)
        i += 1
    print("Training Complete\n")

tf_model = classifier.model
tf_model.evaluate(x_data, y_labels)
