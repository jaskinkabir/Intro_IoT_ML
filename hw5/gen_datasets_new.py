...
def get_label(file_path, label_list, unknown_str = "_unknown"):
...
def wavds2specds(waveform_ds, label_list, verbose=True):
...
def preprocess_dataset(
  ...
  def _get_waveform_and_label(file_path):
      label = _get_label(file_path) # get_label needs access to label_list and unknown_str
      audio_binary = tf.io.read_file(file_path)
      waveform = decode_audio(audio_binary)
      return waveform, label
  ...
  if noisy_reps_of_known is not None:
    ...
    for noise_level in noisy_reps_of_known:
       waveform_ds = waveform_ds.concatenate(copy_with_noise(ds_only_cmds, wave_length_samps, rms_level=noise_level))
  ...
  def _wavds2specds(local_waveform_ds, verbose=True):
      # Get shape from the first element
      try:
          wav, label = next(iter(local_waveform_ds)) # Use iter() to avoid consuming the original ds if it's reusable
          one_spec = _get_spectrogram(wav)
          one_spec = tf.expand_dims(one_spec, axis=0)  # add a 'batch' dimension at the front
          one_spec = tf.expand_dims(one_spec, axis=-1) # add a singleton 'channel' dimension at the back
      except StopIteration:
          print("Warning: Dataset appears to be empty.")
          # Return an empty dataset with the expected structure if possible, or handle error
          # This part might need refinement based on desired empty dataset structure
          return tf.data.Dataset.from_tensor_slices((tf.zeros((0,) + one_spec.shape[1:], dtype=tf.float32),
                                                     tf.zeros((0,), dtype=tf.int64)))


      # Always iterate to count the number of waves
      print("Iterating to count dataset elements...")
      num_waves = 0
      ...
      # Re-initialize the iterator for the main processing loop
      ds_iterator = iter(local_waveform_ds)

      # Iterate using the re-initialized iterator and tqdm
      for wav, label in tqdm(ds_iterator, total=num_waves, desc="Generating Spectrograms"):
...
def gen_datasets_and_save(
    ...
    train_ds = preprocess_dataset(
        ...
        i16min,
        i16max,
        num_silent=num_silent_train,
        noisy_reps_of_known=noisy_reps_of_known,
        unknown_str=unknown_str # Pass unknown_str
    )
    ...
    test_ds = preprocess_dataset(
        ...
        i16min,
        i16max,
        num_silent=num_silent_test,
        unknown_str=unknown_str # Pass unknown_str
    )
    # Remove the print statement accessing len() before datasets are fully processed if using map
    # print(f"Preprocessed datasets: {len(train_ds)} train, {len(val_ds)} val, {len(test_ds)} test")
    # Optional: Shuffle datasets (if not already shuffled in preprocessing)
    # Shuffling large datasets before saving might be memory intensive. Consider shuffling after loading.
    # train_ds = train_ds.shuffle(int(len(train_files)*1.2)) # Example buffer size
